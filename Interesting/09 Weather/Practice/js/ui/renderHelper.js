import { renderHourlyWeather } from './render.js';

export function convertWindDir(deg) {
    let compass = ["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW","N"];
    let index = Math.round((deg % 360) / 22.5 );
    return compass[index];
}

export function getSeparateDaysWeather(list) {
    let separateDays = [];
...
export function subscribeDaysToSelect(extraInfo) {
    ...
    for(let i = 0; i < daysToSelect.length; i++) {
        ...
        // change styles
        daysToSelect[i].onclick = function() {
...
function getFullDaysWeather(list) {
    let fullDaysData = {
        six: [],
        nine: [],
        twelve: [],
